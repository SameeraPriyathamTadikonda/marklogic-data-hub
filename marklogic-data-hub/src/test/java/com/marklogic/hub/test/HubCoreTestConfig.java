...
public class HubCoreTestConfig extends LoggingObject {
    ...
    HubConfigInterceptor hubConfigInterceptor() {
        ...
        HubConfigObjectFactory hubConfigObjectFactory = new HubConfigObjectFactory(gradleProperties);
        ...
        ObjectPool<HubConfigImpl> hubConfigObjectPool = new GenericObjectPool<>(hubConfigObjectFactory, objectPoolConfig);

        return new HubConfigInterceptor(hubConfigObjectPool, gradleProperties);
    }
    ...
    private Properties loadGradleProperties() {
        ...
        loadPropertiesFile("gradle.properties", properties);
        loadPropertiesFile("gradle-local.properties", properties);
        return properties;
    }
    ...
    private void loadPropertiesFile(String filename, Properties properties) {
        ...
        try {
            ...
            logger.warn("Could not read from Gradle properties file: " + file + "; cause: " + ex.getMessage(), ex);
        } finally {
